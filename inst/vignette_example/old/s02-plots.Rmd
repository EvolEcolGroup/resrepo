...
In a R Markdown document within a `resrepo` repository, it easy to find data
by using the function `path_resrepo`:
```{r read_data1}
library(resrepo)
penguins_na_omit <- read.csv(
  path_resrepo(
    "/data/intermediate/s01-preprocess_penguins/penguins_na_omit.csv"
  )
)
```
...
penguins_plot <- plot(
  x = penguins_na_omit$body_mass_g,
