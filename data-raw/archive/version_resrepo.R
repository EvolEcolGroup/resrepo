version_setup <- function() {
  ...
  if (!dir.exists(path_resrepo("versions"))) {
    ...
    fs::dir_copy(
      ...
      path_resrepo("versions/starting/raw")
    )
    fs::dir_copy(
      path_resrepo("data/intermediate"),
      path_resrepo("versions/initial/intermediate")
    )
    # TODO compare the old and new directories to make sure
    # that we can proceed with deleting the old
    ...
    writeLines(
      c("data_resources", "data/raw", "data/intermediate"),
      gitignore_path
    )
  } else {
...
version_add <- function(path = ".", new_version, source_version, description,
                        git_branch = NULL) {
  git_root <- find_git_root()
  ...
  fs::dir_copy(
    path_resrepo(paste("versions/", current_version, sep = "")),
  ...
  data_dir_link(target_dir = path_resrepo(
    paste("versions/", version_name, "/raw", sep = "")
  ), link_dir = "data/raw")
  data_dir_link(
    target_dir = path_resrepo(
      paste("versions/", version_name, "/intermediate", sep = "")
    ),
    link_dir = "data/intermediate"
  )
