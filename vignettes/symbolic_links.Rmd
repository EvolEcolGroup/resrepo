...
vignette: >
  %\VignetteIndexEntry{symbolic links}
...
```{r}
## we don't have this, we need to use data_source_delete
...
data_dir_link(link_dir = "/data/raw/data_input",
              target_dir = file.path(base_dir,"onedrive/gitdata/project1/data/raw/data_input"))
```
...
the git repository. Indeed, if we check whether it exists, we get a positive
answer:
```{r}
...
fs::dir_tree(path=base_dir)
```
However, we can retrieve the file by targetting the directory within our `git`
repository:
```{r}
read.csv(path_resrepo("/data/raw/data_input/new_data_file.csv"))
```

Don't forget that we need to add a data source for this new directory:
```{r}
data_source_add(dir = "/data/raw/data_input",
                source = "onedrive",
                url = "https://my_one_drive/link_to_data_input")
#data_source_check() # this currently fails as we can't delete the default
# data source
```
