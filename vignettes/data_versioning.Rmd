...
`resrepo` allows you to manage data so that they are not tracked by your git repository and create versions of the data directories used in your projects, 
...
be located anywhere on your system (see the end of this vignette for an example).
```{r version_setup}
...
`external_data_storage` folder, which is where the data will be stored.
...
knit_to_results(path_resrepo("/code/s03_pca.Rmd"))
```
There are still no data in the git repository:

```{r check_knitted_external, echo=TRUE}
fs::dir_tree()
```

But the data made it to our external folder:
