...
**Please, note** that the decision of where the data are stored (locally
within the repository or externally) has to be made from the start when
you set up versioning with the `version_setup` function. After this
decision (`version_setup`), all the other methods (i.e. adding new
version) will work in the same way, regardless of where the data are
stored.

# Setting up data versioning

If your data is too big for GitHub and/or you want to version your data, we can use the function `version_setup`. 
**We can only use the function once at the beginning of the project to initialise versioning.** 

**NOTE**: This is the step where you need to decide where data will be 
stored (locally or externally). The command is the same, but depending on your 
choice, you will need to set the `resources_path` argument. This CANNOT be changed later.
...
data storage using the `resource_path` argument in `version_setup`. 
You might want to do this so that your data are backed-up, accessible 
from another machine, or to collaborators. For this scenario, you can refer to
the section below ('Outsourcing the data'), which contains more detailed 
instructions on this fundamental first step of `version_setup`. 
...
`data/intermediate`. Advanced users can also version `data/raw` (see the
end of this vignette). For now, `raw_in_use.meta` will point to the
...
You can specify an external location for your data storage using the `resource_path` argument in `version_setup`.

**NOTE**: Make sure that you have enough space on your hard drive before versioning: 
if you are using a cloud service, a full copy of your data will also be stored 
locally on your hard drive, which reduces the available storage space. If there is not enough space, the new version will not be created.

**Be aware that we can use the `version_setup` function ONLY ONCE**. 
Therefore, for this example, we first create a new (empty) repository that does not yet have any versioning:
...
@TODO currently this works when moving versions from internal to an external location, but it needs updating for moving one external location to another external location.

If once you have versioned your repository, you want to move your data somewhere else, 
and update the links to the new location, you will need to move the `versions` folder 
to the new location, and then re-run `version_setup` with `resources_path` set to the new path.
