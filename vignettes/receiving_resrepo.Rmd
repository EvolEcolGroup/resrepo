...
# TODO
# after this step we would like to create the substructure and copy the data in
...
Now that our `external_data_storage_new` folder contains the data, we can do this
using `version_setup`
```{r}
...
We can now see that we have `data/intermediate` and `data/raw` folders. These 
are both links to the correct version of our data, which are stored externally at 
the `external_data_storage` folder. 
```{r}
fs::dir_tree(external_data_storage_new)
```


We can see that in `external_data_storage` the `starting` and `initial` folders
are now links.
