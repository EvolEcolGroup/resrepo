...
#'
#' Check that all directories in /data/raw and /data/intermediate are found in
#' data_source_list.csv, and that their details are complete.
#'
#' @param path option string giving the path for the root of the repository (NOT
#'   the data directory). If left NULL, then the script assumes that the current
#'   working directory is within the git repository.
#'
#' @export
...
data_source_check <- function(path = NULL) {
  ...
  if (any(is.na(data_sources$source), is.na(data_sources$url))) {
    stop(
      "in data_source_list.csv, some sources/url have been left blank; ",
      "all entries should be filled in, use 'git' in both source and url for ",
