...
version_add <- function(path = ".",
  ...
  # if source versions not given, use the current one,
  # else check that they exist
  # for raw
  ...
  fs::dir_copy(
    path_resrepo(paste("versions/", intermediate_source, sep = "")),
  ...
  if (create_raw) {
    fs::dir_copy(
      path_resrepo(paste("versions/", raw_source, "/raw", sep = "")),
